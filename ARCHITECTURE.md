...
| `ai-course` | Hosted Groq/GPT-OSS course outline, draft generation, version-locked patch proposals, and backend-only Pexels media search. AI-created and AI-edited image/video blocks receive Pexels assets plus attribution when `PEXELS_API_KEY` is configured, while retaining their existing editable fallbacks if search is unavailable. `PexelsMediaProvider` is a singleton that caps real concurrent Pexels HTTP calls at 4 app-wide via an in-process semaphore and de-dupes identical searches with a short-lived in-memory cache (6h on a hit, 10m on a miss), so one large course generation — or several running at once — can't burst through the free-tier hourly quota; the cache is process-local and resets on restart. Full drafts use JSON-object output plus server-side validation because GPT-OSS can omit nullable strict-schema fields. Supports course, lesson, and block scoped edits (`replace_block`, `remove_block`, `insert_blocks`, `replace_lesson`), validates nested edit scopes and non-blank instructions, version-locks against `courseDocumentVersion`, keeps `courseDocument.pages` synchronized upon application, and places inserted lessons in their intended section. Patch proposals ask Groq for one array per allowed op (`buildPatchSchema`/`PATCH_ITEM_SCHEMAS`), each holding only that op's own small, fully-required item shape (filled or left `[]`), rather than either a top-level `anyOf` of differently-shaped alternatives or one shared object with every op's fields nullable — both earlier shapes made gpt-oss's strict-mode structured outputs drop fields instead of nulling them as the schema grew, worst at course scope where a shared object's `required` list was the union of all 6 ops' fields (`json_validate_failed`/missing-properties errors, e.g. `blockId, block, afterBlockId, blocks, changes`). `flattenPatchGroups` re-attaches the `op` tag the model never emits, and `mapPatch` reads whichever fields apply to it. `complete()` now also retries a schema-validation failure, a truncated completion (`finish_reason: 'length'`), or a JSON-parse failure by appending a short corrective instruction to the conversation instead of resending the same prompt unchanged, logging the Groq-reported `failed_generation`/message when available; `max_completion_tokens` and the soft patch-count the model is asked for both scale with scope (course/lesson/block) instead of one flat value for every request. `proposeEdit` sends Groq a lean `buildEditingContext` projection of the course instead of the raw document — dropping `pages` (a full mirror of `lessons`, only resynced locally by `applyProposal` after an edit is applied) and per-block `assetUrl`/`metadata` (regenerated by `enrichBlockMedia`/`mapBlock` regardless of what was sent) — and caps that call's `max_completion_tokens` at 1,200 (block scope) / 2,000 (lesson) / 2,500 (course); left unset, the model's default 65,536-token completion budget is reserved against the account's tokens-per-minute quota before generation starts, which alone can exceed a Groq on_demand-tier TPM cap regardless of actual prompt size. On a 429 (`rate_limit_exceeded`), `complete()` reads Groq's `retry-after` header (falling back to parsing it out of the error message) and auto-retries once for waits of 15s or less; a longer wait is surfaced as a clean `429` `HttpException` instead of Groq's raw message, which includes a billing-upsell link not meant for the educator — see Known issue #5 for the underlying tier limit this works around; see `AI_COURSE_INTEGRATION.md` |
...
Two export-correctness fixes (2026-09-23): `buildManifest`'s `<resource>` tag now emits `adlcp:scormType` (capital T) for a SCORM 2004 export and `adlcp:scormtype` (lowercase) for 1.2 — the two specs use different casing for this attribute, and the previous one-casing-for-both version could fail strict manifest validation on a 2004 package. Separately, the embedded runtime's `renderBlock` used to route `hotspot` blocks through `renderQuestionBlock` (built for the real quiz types — `multiple_choice`/`multiple_select`/`true_false`/`fill_blank`/`matching` — which reads `block.items` as answer options), so an authored hotspot block exported to SCORM as an empty "Question" card with no image and no options. It now has its own `renderHotspotBlock`, mirroring the pattern `ScormPdfService.renderPdfHotspotBlock` already used; PDF export was never affected by this one.
...
- `context/` — `AuthContext` (JWT + minimal user stored in cookies via `js-cookie`, `SameSite=Strict`, `Secure` in production; cross-tab sync through `useSyncExternalStore` plus a custom `edu-auth-change` window event), `SocketContext` (Socket.IO client + a per-scenario "room" hook), `ThemeContext`, `SidebarContext`, `LanguageContext` (i18n locale state in `localStorage` key `edu_locale`, zero-dependency dictionary in `lib/translations.ts`, dynamic `useTranslation` hook).
...
**5. [Low] Groq free/on_demand tier caps `openai/gpt-oss-20b` at a flat 8,000 tokens/minute, org-wide**: even with the trimmed request sizes in the `ai-course` row above, two AI actions in quick succession (e.g. a course draft immediately followed by an "Ask AI" edit) can still push the rolling window past the cap and 429. `GroqCourseProvider.complete()` now retries automatically once for short (≤15s) cooldowns using Groq's `retry-after` header and surfaces a clean message otherwise, but sustained rapid use will still need requests paced roughly a minute apart, or the account moved to Groq's Developer tier for a higher TPM ceiling.

**6. [Low] `consequence`/`decision_recap`/`branch_merge`/`conditional_gate` blocks render plainer in SCORM than in PDF**: all four are selectable in `blockCatalog` ("Branching & Decision" category) and have dedicated renderers in `ScormPdfService` (`renderPdfDialogueBlock`, `renderPdfConditionalGateBlock`), but `ScormBuildService`'s embedded runtime has no special case for them, so they fall through to the generic title+content `interaction-block` fallback — losing `consequence`'s image, `conditional_gate`'s locked message, etc. Not a regression versus authoring: the live editor preview (`PreviewBlock.tsx`) has no special case for these types either (only `choice_point`/`branching_dialogue` do, via `branchingDecisionBlockTypes`), so SCORM matches what the author already saw while building the course — it's PDF that's ahead here, not SCORM that's behind the editor. Fixing this means giving `ScormBuildService.renderBlock` renderers for these four types, following the pattern PDF already uses.
